...
/*   Updated: 2025/06/18 18:21:44 by spunyapr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	- If multiple heredocs are in a single command,
		and SIGINT happens in the first one: Clean up all opened fd_heredoc from other nodes if already opened
	...
	- int get_heredoc(char *delimiter)
		{
			if (get_line == 130)
				return (130) for signal --> also tell main that if get_heredoc == 130
