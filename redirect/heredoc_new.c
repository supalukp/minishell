...
/*
    ...
    2. redirect_heredoc 
        ...
        [x] get_heredoc_line : get input line by line and write to tmp file (fd_write)
        [x] after user type the delimiter the get line stop and close(fd_write) and close(fd_tty)
        [x] also in heredoc_get_line we listen the signal if get sigint(ctrl+c) we close fd and return 130
        [x] if heredoc_get_line return 130 unlink(tmpfile) free(tmpfile) and set signal to default
        [] read tmpfile fd_read = open(tmpfile, O_RDONLY); 
            if (fd_read != -1) set tree->fd_heredoc == fd_read -> unlink(tmpfile) free(tmpfile) set_signal_to_default
    ...
    NOTE:
    - if heredoc get interrupt by signal like sigint the whole line of command return 130
    - Suggestion: validate fd_read >= 0 instead of fd_read != -1 for clarity
    - If multiple heredocs are in a single command, and SIGINT happens in the first one: Clean up all opened fd_heredoc from other nodes if already opened
    ...
    - int get_heredoc(char *delimiter)
        {
            if (get_line == 130)
                return (130) for signal --> also tell main that if get_heredoc == 130 -> exit_status == 130 and don't go to execution
